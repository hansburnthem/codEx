...
@php
    $categories = \App\BookCategory::orderBy('category_name')->get();
@endphp


<body class="bg-gray-100 h-screen antialiased leading-none font-sans p-1">
    <!--Navbar-->
    <div class="absolute left-0 right-0 top-0 p-3 flex justify-between text-white shadow-lg" style="background-color:rgb(251, 161, 76);">

        <!--TItle-->
        <div class="self-center">
            <a href="/" class="no-underline hover:underline text-xl italic duration-300 hover:text-black font-serif" style="color:white"><b>Bukupedia</b></a>
        </div>
        ...
        <ul class="hidden md:flex items-center">
            ...
            <div class="hidden md:flex items-center">
                ...
                <div id="mobile-nav2" class="hidden absolute left-auto w-auto top-16 text-sm text-center">
                    <div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
                        <div class="z-30 relative p-2 text-black flex flex-col " style="color: orange">
                            @if(count($categories))
                            @foreach($categories as $category)
                            <a href="{{URL('viewProduct/'. $category->id)}}" class="p-1 hover:bg-yellow-300 hover:text-white duration-300 rounded-lg" style="color: black">{{ $category->category_name }}</a>
                            @endforeach
                            @else
            ...
            @auth
                <li class="px-2">
                    <button class="text-yellow-500 opacity-100 hover:opacity-50 focus:opacity-50 duration-300 cursor-pointer focus:outline-none p-1 bg-white rounded-lg shadow-lg mx-1 p-2 rounded-lg font-medium w-full shadow-sm" onclick="profileMenu()">
                        {{ auth()->user()->username }}
                    </button>
                        ...
                        <div class="rounded-lg shadow-lg ring-auto ring-black ring-opacity-5 overflow-hidden">
                            <div class="z-30 relative p-2 text-black flex flex-col">
                                ...
                                @if(auth()->user()->role->role_name == 'Manager')
                                    <a class="text-lg text-yellow-300 border border-solid border-yellow-300 rounded-lg" style="color:black" >Manager</a>
                                    <a href="{{ route('add_book') }}" class="p-1 hover:bg-yellow-300 hover:text-white duration-300 rounded-lg mt-1" style="color:black">Add Book</a>
                                    <a href="{{ route('manager_categories') }}" class="p-1 hover:bg-yellow-300 hover:text-white duration-300 rounded-lg" style="color:black">Manage Categories</a>
                                    <!--Navbar for Customer-->
                                    @elseif(auth()->user()->role->role_name == 'Customer')
                                    <a class="text-lg text-yellow-300 border border-solid border-yellow-300 rounded-lg" style="color:black">Customer</a>
                                    <a href="{{ route('view_cart') }}" class="p-1 hover:bg-yellow-300 hover:text-white duration-300 rounded-lg mt-1" style="color:black">My Cart</a>
                                    <a href="{{ route('view_transactions') }}" class="p-1 hover:bg-yellow-300 hover:text-white duration-300 rounded-lg" style="color:black">Transaction History</a>
                                    @endif
                                    <a href="{{ route('change_password') }}" method="post" class="p-1 hover:bg-yellow-300 hover:text-white duration-300 rounded-lg" style="color:black">Change Password</a>
                                    <form action="{{ route('logout') }}" method="post">
                                        @csrf
                                        <button class="p-1 hover:bg-yellow-300 hover:text-white duration-300 rounded-lg mt-1">logout</button>
                                    </form>
                            </div>
                        </div>
                    </div>
